...
void performTraining(int nPoints){
	...
	printf("--level: %d, rank %d, Gaussian kernel: gamma = %f, Cost: %f, Tolerance: %f, Epsilon: %f, #samples: %d (%d positve, %d negtive, pos/neg: %lf)\n", level, rank, -parameterA, cost, tolerance, epsilon, nPoints, pospoints, negpoints, 1.0*pospoints/negpoints); 
...
void printModel(const char* outputFile, DataType* data, int* data_index, int* row_index, DataType* labels, DataType* alpha, int nPoints) { 
  ...
  if (printCoef0) {
	...
	//fclose(out_svs);
	int total_svs=0;
	printf("number of Support Vectors on node %d is %d (pos: %d, neg: %d, pos/neg: %lf)\n", rank, nSVs, pSV, nSV, 1.0*pSV/nSV);
...
int main(int argc, char** argv)  {   
  ...
  if (outputFilename == NULL) {
	...
	int* membership = (int*) malloc(sub_nPoints * sizeof(int));
    	...
    	assert(posClusterSize != NULL);
    	int * negClusterSize = (int*) calloc(numClusters, sizeof(int));
    	assert(negClusterSize != NULL);
	...
	int pos_balanced = pospoints/numClusters;
	int neg_balanced = negpoints/numClusters;
    int balanced = sub_nPoints/numClusters;
    printf("on node %d, sub_nPoints: %d, pospoints: %d, negpoints: %d, balanced: %d, pos_balanced: %d, neg_balanced: %d\n", rank, sub_nPoints, pospoints, negpoints, balanced, pos_balanced, neg_balanced);
    for (int i=0; i<sub_nPoints; i++) {
	    if((int)labels[i]==1){
		    float mindis = INF;
			int minind = 0;
			for (int j=0; j<numClusters; j++){
				float dist = euclid_dist_2(i, max_index, clusters[j]);
				if(dist < mindis && posClusterSize[j] < pos_balanced+1){
					mindis = dist;
					minind = j;
				}
			}
			...
			for (int j=0; j<numClusters; j++){
				float dist = euclid_dist_2(i, max_index, clusters[j]);
				if(dist < mindis && negClusterSize[j] < neg_balanced+1){
					mindis = dist;
					minind = j;
				}
			}
    ...
    /*for (int i=0; i<sub_nPoints; i++) {
		...
		for (int j=0; j<numClusters; j++){
			float dist = euclid_dist_2(i, max_index, clusters[j]);
			if(dist < mindis && newClusterSize[j] < balanced){
				mindis = dist;
				minind = j;
			}
		}
		newClusterSize[minind]++;
		membership[i] = minind;
    }*/
    ...
    if (rank == 0) printf("rank %2d: easy kmeans is finished, time=%f sec\n",rank,curT);
  ...
  if (kType == LINEAR) {
	...
	if (rank==0)	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@-casvm fcfs time is %lf sec, fcfs kmeans time is %lf, total time is %lf\n\n", dcsvm_time, curT, (dcsvm_end-curT0));

...
int main(int argc, char** argv)  {   
  ...
  if (outputFilename == NULL) {
    ...
    for(int j=0;j<numClusters;j++){
		while(newClusterSize[j]>balanced){
			int maxdist = 0;
			int maxind = 0;
			for(int i=0;i<sub_nPoints;i++){
			...
			for(int k=0;k<numClusters;k++){
				if(dist[maxind][k]<mindist&&newClusterSize[k]<balanced){
    ...
    //if(rank==0)	printf("The number of elements on node %d is:\n", rank);
	//for (int l=0; l<numClusters; l++)	if(rank==0)	printf("%d ", newClusterSize[l]);
	//if(rank==0)	printf("\n");
	free(dist[0]);
	free(dist);
    MPI_Allreduce(newClusterSize, clusterSize, numClusters, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  ...
  if (kType == LINEAR) {
	...
	if (rank==0)	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@-dcsvm time is %lf sec, kmeans (%d loops) time is %lf, total time is %lf\n\n", dcsvm_time, loop, curT, (dcsvm_end-curT0));

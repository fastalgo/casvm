...
void printModel(const char* outputFile, DataType* data, int* data_index, int* row_index, DataType* labels, DataType* alpha, int nPoints) { 
  ...
  if (printCoef0) {
	...
	//fclose(out_svs);
	int total_svs=0;
	printf("number of Support Vectors on node %d is %d (pos: %d, neg: %d, ratio: %lf)\n", rank, nSVs, pSV, nSV, 1.0*pSV/nSV);
...
int main(int argc, char** argv)  {   
  ...
  while (1) {
    ...
    case 0:
      break;
    case 'v':
      printf("casvm fcfs with pos/neg ratio version: 1.0\n");
      return(0);
    case 'f':
      printHelp();
      return(0);
  ...
  if (kType == LINEAR) {
	...
	if (rank==0)	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@-casvm_fcfs time is %lf sec, fcfs kmeans time is %lf, total time is %lf\n\n", dcsvm_time, curT, (dcsvm_end-curT0));

...
{   
  	...
  	while (1) 
	{
    		...
    		switch (currentOption) 
		{
    			case 0:
      				break;
    			case 'v':
      				printf("CASVM Random version: 1.0\n");
      				return(0);
    			case 'f':
      				printHelp();
      				return(0);
	...
	DataType** newClusters    = (DataType**) malloc(numClusters *             sizeof(DataType*));
    	assert(clusters != NULL);
	assert(newClusters != NULL);
    	clusters[0] = (DataType*)  calloc(numClusters * max_index, sizeof(DataType));
	newClusters[0] = (DataType*)  calloc(numClusters * max_index, sizeof(DataType));
	...
	for (int pi=0; pi<sub_nPoints; pi++) 
	{
		...
		{
                	newClusters[rank][data_index[j]] += data[j];
		}
    	}
	...
	// for(int i=0;i<numClusters;i++)	clusterSize[i] = sub_nPoints;
	
	for (int i=0; i<numClusters; i++) 
	{
        	for (int j=0; j<max_index; j++) 
		{
            		clusters[i][j] /= sub_nPoints;	//clusterSize[i];   
        	}
    	}
	...
	MPI_Barrier(MPI_COMM_WORLD);
    	...
    	if (rank == 0) 
	{
		printf("rank %2d: partition is finished, time=%f sec\n", rank, curT);
	}
	...
	if (rank==0)	
	{
		printf("\n@@@@@@@@@@@@@@@@@@@@@@@@-dcsvm time is %lf sec, partition time is %lf, total time is %lf\n\n", dcsvm_time, curT, (dcsvm_time + curT));
	}

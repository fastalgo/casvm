...
void performTraining(int nPoints){
	int pospoints = 0, negpoints = 0;
	for(int i=0;i<nPoints;i++){
		if((int)labels[i]==1)	pospoints++;
		else if((int)labels[i]==-1) negpoints++;
	}
  ...
  gettimeofday(&start, 0);	
	...
	while(bLow > bHigh + 2*tolerance){	  
	  ...
	  for(i=0;i<nPoints;i++){//#pragma vector always aligned //num_threads(MY_THREADS)
		  ...
		  /*if(i==(535)&&iteration<=1000){
		  if(rank==3)	fprintf(debugfile, "i: %d, iter: %d, bHigh: %lf, bLow: %lf, iHigh: %d iLow: %d, devF: %lf, alphaHighDiff: %lf, labels[iHigh]: %lf, highKernel[i]: %lf, alphaLowDiff: %lf, labels[iLow]: %lf, lowKernel[i]: %lf\n", i, iteration, bHigh, bLow, iHigh, iLow, devF[i], alphaHighDiff, labels[iHigh], highKernel[i], alphaLowDiff, labels[iLow], lowKernel[i]);
		  }*/
	  	}
...
float euclid_dist_2(int id, int max_index, float * center)
{
    ...
    for(int i=0;i<max_index;i++)	ans += center[i]*center[i]; 

    for(int i=row_index[id];i<row_index[id+1];i++)	ans += data[i] * data[i];

    for(int i=row_index[id];i<row_index[id+1];i++)	ans -= 2* data[i] * center[data_index[i]];
...
int main(int argc, char** argv)  {   
  ...
  if (outputFilename == NULL) {
	...
	if(rank==0){
		...
		for(int i=0;i<numtasks;i++){
			...
			if(i==0)	displs[i] = 0;
		   	else displs[i] = displs[i-1]+send_counts[i-1];
		}
	}
	...
	int* membership = (int*) malloc(sub_nPoints * sizeof(int));
    	...
    	DataType    delta;          /* % of objects change their clusters */
    	DataType    delta_tmp;
    	DataType  **newClusters;    /* [numClusters][numCoords] */
    	for (int i=0; i<sub_nPoints; i++) membership[i] = -1;
    ...
    do {
        ...
        for (int pi=0; pi<sub_nPoints; pi++) {
            ...
            if (membership[pi] != index) delta += 1.0;
	...
	if(sub_nPoints%numClusters!=0) printf("%d can't be divided by %d evenly!!!!!!!\n", sub_nPoints, numClusters);
    float ** dist = (float**) malloc(sub_nPoints*sizeof(float*));
    dist[0] = (float *) malloc(sub_nPoints*numClusters*sizeof(float));
    for (int i=1; i<sub_nPoints; i++)
        dist[i] = dist[i-1] + numClusters;
    int * posclusterSize = (int *) calloc(numClusters, sizeof(int));
    int * negclusterSize = (int *) calloc(numClusters, sizeof(int));
    //int balanced = sub_nPoints/numClusters;
    int posnum = 0;
    int negnum = 0;
    for(int i=0; i<sub_nPoints; i++){
	    if((int)labels[i]==1){
		    posclusterSize[membership[i]]++;
		    posnum++;
	    }else if((int)labels[i]==-1){
		    negclusterSize[membership[i]]++;
		    negnum++;
	    }	
	    for(int j=0; j<numClusters; j++){
		    dist[i][j] = euclid_dist_2(i, max_index, clusters[j]);
	    }
    }
    int pos_balanced = posnum/numClusters;
    int pos_balanced_max = posnum - pos_balanced*numClusters + pos_balanced;
    int neg_balanced = negnum/numClusters;
    int neg_balanced_max = negnum - neg_balanced*numClusters + neg_balanced;
    printf("posnum: %d --- pos_balanced: %d --- pos_balanced_max: %d --- negnum: %d --- neg_balanced: %d --- neg_balanced_max: %d\n", posnum, pos_balanced, pos_balanced_max, negnum, neg_balanced, neg_balanced_max);
    for(int j=0;j<numClusters;j++){
	    //if(rank==0)	printf("pos_balanced: %d - pos_balanced_max: %d - posclusterSize[%d]: %d - neg_balanced: %d - neg_balanced_max: %d - negclusterSize[%d]: %d\n", pos_balanced, pos_balanced_max, j, posclusterSize[j], neg_balanced, neg_balanced_max, j, negclusterSize[j]);
		while(posclusterSize[j]>pos_balanced_max){
			int maxdist = 0;
			int maxind = 0;
			for(int i=0;i<sub_nPoints;i++){
				if(dist[i][j]>maxdist&&membership[i]==j&&(int)labels[i]==1){
					maxdist = dist[i][j];
					maxind = i;
				}
			}
			...
			for(int k=0;k<numClusters;k++){
				if(k!=j&&dist[maxind][k]<mindist&&posclusterSize[k]<pos_balanced){
			...
			posclusterSize[j]--;
			posclusterSize[minind]++;
		}
		while(negclusterSize[j]>neg_balanced_max){
			int maxdist = 0;
			int maxind = 0;
			for(int i=0;i<sub_nPoints;i++){
				if(dist[i][j]>maxdist&&membership[i]==j&&(int)labels[i]==-1){
			...
			int minind = j;
			for(int k=0;k<numClusters;k++){
				if(k!=j&&dist[maxind][k]<mindist&&negclusterSize[k]<neg_balanced){
					mindist = dist[maxind][k];
					minind = k;
				}
			}
			membership[maxind] = minind;
			negclusterSize[j]--;
			negclusterSize[minind]++;
		}
		//if(rank==0)	printf("pos_balanced: %d - pos_balanced_max: %d - posclusterSize[%d]: %d - neg_balanced: %d - neg_balanced_max: %d - negclusterSize[%d]: %d\n", pos_balanced, pos_balanced_max, j, posclusterSize[j], neg_balanced, neg_balanced_max, j, negclusterSize[j]);
    }
	for(int i=0;i<numClusters;i++){
		newClusterSize[i] = posclusterSize[i] + negclusterSize[i];
		//if(rank==0) printf("clusterSize[%d]: %d\n", i, clusterSize[i]);
	}
	free(dist[0]);
	free(dist);
    MPI_Allreduce(newClusterSize, clusterSize, numClusters, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    if (rank == 0) printf("rank %2d: kmeans is finished, loops=%d time=%f sec\n",rank,loop,curT);
  ...
  if (kType == LINEAR) {
	...
	if (rank==0)	printf("\n@@@@@@@@@@@@@@@@@@@@@@@@-casvm bkm time is %lf sec, bkm (%d loops) time is %lf, total time is %lf\n\n", dcsvm_time, loop, curT, (dcsvm_end-curT0));
	...
	char filename[32];
	int i, j;
	for(i=0;outputFilename[i]!='\0';i++) filename[i] = outputFilename[i];
	for(j=0;buffer[j]!='\0';j++, i++) filename[i] = buffer[j];
	filename[i] = '\0';
